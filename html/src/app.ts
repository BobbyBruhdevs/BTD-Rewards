...
interface Reward { name: string; description: string; time: number; claimed: boolean; }
interface PlayerData { activeTime: number; rewards: Reward[]; }
interface MessageData { action: string; data?: any; }

let playerData: PlayerData = { activeTime: 0, rewards: [] };
let currentCode = '';
...
const post = (endpoint: string, body: Record<string, unknown> = {}) => fetch(`https://${resourceName}/${endpoint}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
...
function formatRequired(minutes: number): string {
    if (minutes >= 60) {
...
function show(id: string): void { $(id).classList.add('is-visible'); $(id).setAttribute('aria-hidden', 'false'); }
function hide(id: string): void { $(id).classList.remove('is-visible'); $(id).setAttribute('aria-hidden', 'true'); }
...
function openRewards(data: PlayerData): void { playerData = data; show('app'); hide('codePage'); renderRewards(); }
function closeUi(notifyGame: boolean = true): void { hide('app'); hide('codePage'); if (notifyGame) post('close'); }
function showCode(data: { code: string; rewardName: string; uiColor?: string }): void { if (data.uiColor) applyConfig(data.uiColor); currentCode = data.code; $('codeText').textContent = data.code; $('codeReward').textContent = data.rewardName; $('claimedTime').textContent = new Date().toLocaleString(); hide('app'); show('codePage'); }
...
function resolveRgb(color: string): number[] {
...
function mixColor(color: string, base: string, amount: number): string {
...
function applyConfig(uiColor = uiConfig.uiColor): void {
    ...
    document.querySelectorAll<HTMLElement>('[data-copy]').forEach(element => { const key = element.dataset.copy as keyof typeof uiConfig.copy; element.textContent = String(uiConfig.copy[key]); });
}
...
window.addEventListener('message', (event: MessageEvent<MessageData>) => {
    if (event.data.action === 'open' && event.data.data) { if (event.data.data.uiColor) applyConfig(event.data.data.uiColor); openRewards(event.data.data as PlayerData); }
